...
import urllib.parse
from bose import *
from src.scrape_gmaps_places import ScrapeGmapsPlaces
...
def clean(data_list, query):
    ...
    # Append the keyword to each result
    for result in new_results:
        result['keyword'] = query['keyword']

    # Sort the dictionary keys
    sorted_results = [dict(sorted(result.items())) for result in new_results]

    return sorted_results

# Class untuk scrape link tempat dari Google Maps
class ScrapeGmapsLinks(BaseTask):

    task_config = TaskConfig(output_filename="all", log_time=False, close_on_crash=True)

    browser_config = BrowserConfig(block_images_fonts_css=True, headless=True)

    def save_google(self, driver: BoseDriver, data): # Sets up the data for scraping
        data = {
            "links": data,
            "query": {
                "keyword": "rama",
        ...
        return ScrapeGmapsPlaces().run(driver, data) # run method of the ScrapeGmapsPlaces
    ...
    def run(self, driver, data):
        ...
        # Get the links from Google Maps
        def get_links():
        ...
        # google maps sometimes give duplicate links
        ...
        # Write the results to a JSON and CSV file
